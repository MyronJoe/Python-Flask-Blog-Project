...
def register():
    ...
    if form.validate_on_submit():
        hashes_pw = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        user = User(username=form.username.data, email=form.email.data, password=hashes_pw)
        db.session.add(user)
        db.session.commit()
        flash('Your account have been created successful! kindly Login', 'success')
...
def delete_users(user_id):
    ...
    return redirect(url_for('all_users', user_id=users.id))
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('home'))
    return render_template('create_post.html', form=form, legend='Creat New Post')
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        hashes_pw = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        user.password = hashes_pw
        db.session.commit()
        flash('Your password has been updated successful! kindly Login', 'success')
        return redirect(url_for('login'))
